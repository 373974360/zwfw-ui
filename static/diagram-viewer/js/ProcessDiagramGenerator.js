...
var ProcessDiagramGenerator = {
    ...
    _drawDiagram: function () {
        ...
        if (processDiagram != undefined && processDiagram != null) {
            ...
            //切换面包屑
            var ul = $("#" + ProcessDiagramGenerator.options.diagramBreadCrumbsId).find("ul");
            ul.find('li[processDefinitionId="' + processDefinitionId + '"]').click();
            return;
        } else {
    ...
    addBreadCrumbsItem: function (processDefinitionId) {
        ...
        //console.log("liListNew size: ", liListNew.length);
        var values = {
            id: 'breadCrumbsItem_' + liListNew.length,
    ...
    showFlowInfo: function (flow) {
        ...
        var TPL_FLOW_INFO = '<tr><td>ID</td><td>{flow}</td></tr>'
            + '<tr><td>名称</td><td>{name}</td></tr>'
            + '<tr><td>来源ID</td><td>{sourceActivityId}</td></tr>'
            + '<tr><td>目的ID</td><td>{destinationActivityId}</td></tr>'
        //+ '<tr><td>是否默认流</td><td>{isDefault}</td></tr>'
        //+ '<tr><td>有条件</td><td>{isConditional}</td></tr>'
        //+ '<tr><td>突出显示</td><td>{isHighLighted}</td></tr>';
        var tpl = '<div><h2>流向说明:</h2><table class="table">' + Lang.sub(TPL_FLOW_INFO, values) + '</table></div>';
        //console.log("info: ", tpl);
        diagramInfo.html(tpl);
    },
    getEntityName: function (type) {
        return (
            {
                startEvent: '起点',
                endEvent: '终点',
    ...
    showActivityInfo: function (activity) {
        ...
        var values = {
            ...
            documentation: activity.getProperty("documentation") || ''
        };
        var TPL_ACTIVITY_INFO = ''
            + '<tr><td>ID:</td><td>{activityId}</td></tr>'
            + '<tr><td>类型:</td><td>'+ this.getEntityName(values.type) + '</td></tr>'
            + '<tr><td>描述:</td><td>{documentation}</td></tr>';
        var TPL_CALLACTIVITY_INFO = ''
            + '<tr><td>collapsed</td><td>{collapsed}</td></tr>'
            + '<tr><td>processDefinitonKey</td><td>{processDefinitonKey}</td></tr>';

        var template = '<div><h2>' +values['name']+'</h2><table class="table" ">' + TPL_ACTIVITY_INFO;
